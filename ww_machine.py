...
WWURL     =     r'http://bbs.uestc.edu.cn/forum.php?mod=post&action=reply&fid=388&tid=1570810&extra=&replysubmit=yes&mobile=yes'
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not my_account.login(username, password,mobile=True):
            print 'login failed'
        else:
            print 'login succeed'
            #lastreply_pattern = re.compile(r'div id="post_\d+".*?postnum.*?em>.*?(\d+)',re.S)
            lastreply_pattern = re.compile(r'<em>(\d{1,5})</em>',re.S)
                    ...
                    if lastreply_result:
                        if mode == '1':
                            if (len(lastreply_result) == 7):
                                print "reply 8"
                                my_account.reply_ql(WWURL,content3[random.randint(0,1)])
                                time.sleep(5)
